...
def load_historical_weather(file_path='historical_weather.csv'):
    ...
    return pd.DataFrame(columns=['date', 'temperature_2m_min', 'snowfall_sum', 'precipitation_sum'])
...
def generate_html(location, driver_status_string, first_snow_message, seasonal_message, last_prediction_date, next_prediction_date, current_weather, plot_path):
...
if __name__ == "__main__":
    ...
    if location is None:
        location = "Lansing, MI"  # Default to Lansing, Michigan
    ...
    while True:
        try:
            ...
            model_1_data, model_2_data, historical_doys, current_drivers = get_historical_data_for_ml(lat, lon)
            
            if len(historical_doys) == 0:
            ...
            adjusted_doys = [d if d < 365 else d - 365 for d in historical_doys]
            ...
            if nws_snow_date:
                ...
                if ml_unwrapped is None:
                    ...
                    first_snow_message = f"First Snowfall: {predicted_date.strftime('%B %d, %Y')} (Days from now: {days_until}) Confidence: MEDIUM (Based on {len(historical_doys)} years)"
            ...
            # Generate HTML
            generate_html(location, driver_status_string, first_snow_message, seasonal_message, last_prediction_date, next_prediction_date, current_weather, plot_path)
